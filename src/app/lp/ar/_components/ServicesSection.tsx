"use client"

import React from "react";

interface ServicesSectionProps {
    onBookNow: () => void;
}

const SERVICES = [
    {
        label: 'أطفال الأنابيب / الحقن المجهري',
        description:
            'عمليات أطفال الأنابيب والحقن المجهري تُعد من أبرز علاجات الخصوبة المتقدمة، والمصممة لمساعدة الأزواج الذين يواجهون تحديات في الإنجاب.',
        icon: (
            <svg xmlns="http://www.w3.org/2000/svg" xmlSpace="preserve" width="506px" height="512px" className="h-6 w-6" version="1.1" shapeRendering="geometricPrecision" textRendering="geometricPrecision" imageRendering="optimizeQuality" fillRule="evenodd" clipRule="evenodd" viewBox="0 0 506 512.33" xmlnsXlink="http://www.w3.org/1999/xlink" >
                <path fill="currentColor" fillRule="nonzero" d="M272.97 80.39c42.09,0 80.26,17.09 107.89,44.71l0.06 0.06c27.61,27.63 44.69,65.78 44.69,107.87 0,42.09 -17.09,80.25 -44.71,107.89l-0.06 0.06c-18,17.99 -40.47,31.5 -65.57,38.74 -0.61,-7.24 -2.22,-14.27 -4.84,-20.93 21.18,-6.29 40.1,-17.77 55.29,-32.97 23.76,-23.74 38.43,-56.54 38.43,-92.79 0,-36.25 -14.67,-69.05 -38.41,-92.78 -23.72,-23.73 -56.52,-38.4 -92.77,-38.4 -36.25,0 -69.05,14.67 -92.75,38.38 -19.68,19.68 -33.13,45.56 -37.16,74.47 -7.88,4.17 -15.52,9.25 -22.7,15.23 0.82,-40.87 17.74,-77.84 44.68,-104.79l0.06 -0.07c27.64,-27.61 65.78,-44.68 107.87,-44.68zm47.11 148.98c14.31,0 25.9,11.59 25.9,25.89 0,14.31 -11.59,25.9 -25.9,25.9 -14.3,0 -25.89,-11.59 -25.89,-25.9 0,-14.3 11.59,-25.89 25.89,-25.89zm-72.91 -76.38c18.55,0 33.59,15.04 33.59,33.59 0,18.54 -15.04,33.59 -33.59,33.59 -18.54,0 -33.58,-15.05 -33.58,-33.59 0,-18.55 15.04,-33.59 33.58,-33.59zm-31.75 111.44c18.19,18.19 12.43,53.45 -12.88,78.76 -20.84,20.84 -48.45,28.43 -67.7,20.28 -15.64,1.63 -29,7.67 -36.16,15.96 -30.05,34.79 1.42,51.94 -35.26,86.16 -12.38,11.55 -28.59,19.42 -44.66,20.29 -6.23,0.35 -12.48,-0.09 -18.76,-1.41 41.27,-6.83 58.53,-24.27 65.64,-42.66 10.33,-26.69 -3.43,-42.28 16.64,-67.34 9.62,-12.01 21.46,-20.76 35.21,-27.07 -9.87,-19.31 -2.55,-48.36 19.18,-70.09 25.31,-25.31 60.56,-31.07 78.75,-12.88zm37.56 113.14c-11.38,-11.37 -33.44,-7.77 -49.27,8.07 -13.04,13.04 -17.78,30.3 -12.7,42.34 -1.01,9.78 -4.79,18.14 -9.98,22.62 -21.76,18.8 -32.49,-0.89 -53.89,22.06 -7.22,7.75 -12.14,17.88 -12.7,27.93 -0.2,3.9 0.07,7.82 0.89,11.74 4.27,-25.81 15.19,-36.61 26.68,-41.07 16.7,-6.45 26.45,2.16 42.13,-10.4 7.51,-6.01 12.98,-13.42 16.95,-22.03 12.07,6.18 30.23,1.59 43.82,-11.98 15.84,-15.84 19.44,-37.9 8.07,-49.28zm19.99 -377.57c64.34,0 122.61,26.09 164.77,68.25 42.17,42.17 68.26,100.43 68.26,164.78 0,64.34 -26.09,122.61 -68.26,164.77 -42.16,42.17 -100.43,68.26 -164.77,68.26l-2.1 -0.02c5.53,-3.66 10.8,-7.94 15.67,-12.81l0.2 -0.19c3.06,-3.05 5.88,-6.27 8.47,-9.6 49.54,-5.18 94,-27.46 127.36,-60.82 38.28,-38.27 61.96,-91.17 61.96,-149.59 0,-58.42 -23.68,-111.32 -61.96,-149.6 -38.28,-38.28 -91.18,-61.96 -149.6,-61.96 -58.42,0 -111.32,23.68 -149.6,61.96 -38.27,38.28 -61.96,91.18 -61.96,149.6 0,26.53 4.89,51.93 13.81,75.34 -0.38,2.66 -0.65,5.33 -0.82,7.98 -4.69,3.97 -9.21,8.38 -13.56,13.24 -13.41,-29.42 -20.9,-62.12 -20.9,-96.56 0,-64.35 26.09,-122.61 68.26,-164.78 42.16,-42.16 100.43,-68.25 164.77,-68.25z" />
            </svg>
        ),
    },
    {
        label: 'اختيار جنس الجنين',
        description:
            'يُعد اختيار جنس الجنين أحد الخيارات الطبية المتاحة ضمن رحلة العلاج بأطفال الأنابيب، حيث يتم فحص الأجنة قبل الإرجاع لتحديد تركيبها الكروموسومي، بما في ذلك الكروموسومات المحددة للجنس.',
        icon: (
            <svg
                version="1.1"
                className="w-6 h-6"
                xmlns="http://www.w3.org/2000/svg"
                xmlnsXlink="http://www.w3.org/1999/xlink"
                viewBox="0 0 32 32"
                xmlSpace="preserve"
                fill="currentColor"
            >
                <g id="SVGRepo_iconCarrier">

                    <line
                        style={{ fill: 'none', stroke: 'currentColor', strokeWidth: 2, strokeMiterlimit: 10 }}
                        x1={4}
                        y1="22.583"
                        x2="9.417"
                        y2={28}
                    />
                    <polyline
                        style={{ fill: 'none', stroke: 'currentColor', strokeWidth: 2, strokeMiterlimit: 10 }}
                        points="22,4 28,4 28,10 "
                    />
                    <line
                        style={{ fill: 'none', stroke: 'currentColor', strokeWidth: 2, strokeMiterlimit: 10 }}
                        x1={28}
                        y1={4}
                        x2="22.232"
                        y2="9.768"
                    />
                    <line
                        style={{ fill: 'none', stroke: 'currentColor', strokeWidth: 2, strokeMiterlimit: 10 }}
                        x1={4}
                        y1={28}
                        x2="9.454"
                        y2="22.546"
                    />
                    <path d="M15.86,11.255c-0.114,0.087-0.232,0.167-0.334,0.27c-0.433,0.433-0.723,0.964-0.883,1.538 c1.09,0.14,2.102,0.612,2.892,1.402C18.479,15.409,19,16.665,19,18s-0.521,2.591-1.465,3.536S15.335,23,14,23 s-2.591-0.52-3.535-1.464S9,19.335,9,18c0-0.865,0.239-1.686,0.65-2.421C9.553,15.063,9.5,14.536,9.5,14 c0-0.498,0.05-0.987,0.133-1.467c-0.201,0.162-0.397,0.332-0.582,0.517C7.729,14.373,7,16.13,7,18s0.729,3.627,2.051,4.95 S12.13,25,14,25s3.627-0.728,4.949-2.05S21,19.87,21,18s-0.729-3.627-2.051-4.95C18.078,12.179,17.016,11.572,15.86,11.255z" />
                    <path d="M22.949,9.05C21.627,7.728,19.87,7,18,7s-3.627,0.728-4.949,2.05S11,12.13,11,14s0.729,3.627,2.051,4.95 c0.871,0.871,1.933,1.478,3.089,1.795c0.114-0.087,0.232-0.167,0.334-0.27c0.433-0.433,0.723-0.964,0.883-1.538 c-1.09-0.14-2.102-0.612-2.892-1.402C13.521,16.591,13,15.335,13,14s0.521-2.591,1.465-3.536S16.665,9,18,9s2.591,0.52,3.535,1.464 S23,12.665,23,14c0,0.865-0.239,1.686-0.65,2.421c0.096,0.516,0.15,1.043,0.15,1.579c0,0.498-0.05,0.987-0.133,1.467 c0.201-0.162,0.397-0.332,0.582-0.517C24.271,17.627,25,15.87,25,14S24.271,10.373,22.949,9.05z" />
                </g>
            </svg>
        ),
    },

    // {
    //     label: 'تنظيم الأسرة ',
    //     description: 'إرشادات شاملة ودعم طبي لتوازن عائلتك بما يتناسب مع احتياجاتك.',
    //     icon: (
    //         <svg
    //             fill="currentColor"
    //             className="w-6 h-6"
    //             version="1.1"
    //             xmlns="http://www.w3.org/2000/svg"
    //             xmlnsXlink="http://www.w3.org/1999/xlink"
    //             viewBox="0 0 266.578 266.578"
    //             xmlSpace="preserve"
    //         >
    //             <path stroke="currentColor" strokeWidth={10} paintOrder={"stroke fill"} d="M38.136,61.876c15.093,0,27.372-12.278,27.372-27.371c0-15.095-12.278-27.375-27.372-27.375 c-15.092,0-27.371,12.28-27.371,27.375C10.765,49.598,23.044,61.876,38.136,61.876z M38.136,16.175 c10.105,0,18.326,8.223,18.326,18.33c0,10.104-8.221,18.326-18.326,18.326c-10.104,0-18.326-8.221-18.326-18.326 C19.81,24.398,28.032,16.175,38.136,16.175z" />
    //             <path stroke="currentColor" strokeWidth={10} paintOrder={"stroke fill"} d="M11.787,173.464v81.461c0,2.498,2.025,4.523,4.522,4.523c2.498,0,4.522-2.024,4.522-4.523v-83.79 c0-0.069-0.007-0.138-0.01-0.207c0.007-1.624-0.861-3.199-2.395-4.016c-5.786-3.081-9.38-9.04-9.38-15.553v-49.717 c0-9.703,7.892-17.596,17.593-17.596H49.82c9.701,0,17.593,7.894,17.593,17.596v49.717c0,6.511-3.597,12.471-9.386,15.553 c-1.565,0.833-2.435,2.454-2.392,4.113c-0.001,0.037-0.005,0.072-0.005,0.109v83.79c0,2.498,2.025,4.523,4.523,4.523 c2.498,0,4.522-2.024,4.522-4.523v-81.468c7.332-4.935,11.784-13.171,11.784-22.097v-49.717c0-14.69-11.95-26.642-26.638-26.642 H26.639C11.95,75.002,0,86.954,0,101.644v49.717C0,160.292,4.453,168.531,11.787,173.464z" />
    //             <path stroke="currentColor" strokeWidth={10} paintOrder={"stroke fill"} d="M124.779,123.669c15.095,0,27.375-12.282,27.375-27.377c0-15.095-12.28-27.375-27.375-27.375 c-15.092,0-27.371,12.28-27.371,27.375C97.408,111.387,109.687,123.669,124.779,123.669z M124.779,77.962 c10.107,0,18.33,8.223,18.33,18.329c0,10.108-8.223,18.332-18.33,18.332c-10.104,0-18.326-8.224-18.326-18.332 C106.454,86.185,114.675,77.962,124.779,77.962z" />
    //             <path stroke="currentColor" strokeWidth={10} paintOrder={"stroke fill"} d="M105.559,211.485v43.44c0,2.498,2.025,4.523,4.523,4.523c2.498,0,4.522-2.024,4.522-4.523v-45.768 c0-0.07-0.007-0.138-0.01-0.207c0.007-1.624-0.861-3.198-2.394-4.015c-5.789-3.084-9.385-9.044-9.385-15.553v-25.952 c0-9.703,7.893-17.596,17.593-17.596h8.925c9.701,0,17.593,7.893,17.593,17.596v25.952c0,6.509-3.597,12.468-9.385,15.554 c-1.564,0.833-2.434,2.454-2.391,4.113c0,0.036-0.005,0.071-0.005,0.108v45.768c0,2.498,2.025,4.523,4.523,4.523 c2.498,0,4.523-2.024,4.523-4.523v-43.447c7.329-4.939,11.78-13.173,11.78-22.096v-25.952c0-14.69-11.95-26.641-26.638-26.641 h-8.925c-14.688,0-26.638,11.951-26.638,26.641v25.952C93.769,198.311,98.224,206.549,105.559,211.485z" />
    //             <path stroke="currentColor" strokeWidth={10} paintOrder={"stroke fill"} d="M216.367,76.136c15.093,0,27.371-12.277,27.371-27.369c0-15.099-12.278-27.383-27.371-27.383 c-15.094,0-27.374,12.284-27.374,27.383C188.993,63.859,201.273,76.136,216.367,76.136z M216.367,30.429 c10.105,0,18.326,8.227,18.326,18.338c0,10.103-8.222,18.323-18.326,18.323c-10.107,0-18.329-8.22-18.329-18.323 C198.038,38.656,206.26,30.429,216.367,30.429z" />
    //             <path stroke="currentColor" strokeWidth={10} paintOrder={"stroke fill"} d="M170.675,189.69h26.467v65.236c0,2.498,2.025,4.523,4.523,4.523c2.498,0,4.523-2.024,4.523-4.523v-69.759 c0-0.313-0.032-0.618-0.092-0.912c-0.031-0.147-0.067-0.292-0.112-0.433c-0.573-1.841-2.29-3.178-4.319-3.178 c-0.001,0-0.001,0-0.002,0h-23.67l20.495-40.994c0.314-0.627,0.478-1.32,0.478-2.022v-21.926c0-9.592,7.807-17.396,17.402-17.396 c9.594,0,17.399,7.804,17.399,17.396v21.926c0,0.703,0.163,1.395,0.477,2.022l20.495,40.994h-23.485c0,0-0.001,0-0.002,0 c-2.029,0-3.747,1.337-4.319,3.178c-0.044,0.142-0.081,0.286-0.111,0.433c-0.061,0.294-0.092,0.599-0.092,0.912v69.759 c0,2.498,2.024,4.523,4.522,4.523c2.498,0,4.523-2.024,4.523-4.523V189.69h26.282c1.567,0,3.023-0.812,3.848-2.145 c0.823-1.333,0.898-2.998,0.198-4.4l-23.291-46.585v-20.859c0-14.58-11.862-26.441-26.443-26.441 c-14.583,0-26.447,11.861-26.447,26.441v20.859l-23.29,46.585c-0.701,1.402-0.626,3.066,0.198,4.4 C167.652,188.878,169.108,189.69,170.675,189.69z" />
    //         </svg>
    //     ),
    // },

    {
        label: 'الفحوصات الجينية للأجنة قبل الإرجاع',
        description: 'فحوصات متقدمة للكشف عن الاضطرابات الكروموسومية لاختيار الأجنة السليمة للإرجاع.',
        icon: (
            <svg className="h-6 w-6" height="200px" width="200px" version="1.1" id="_x32_" xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink" viewBox="0 0 512 512" xmlSpace="preserve" fill="currentColor">
                <path className="st0" d="M351.815,128.59c-1.797-18.117-1.44-34.865,2.855-50.696c2.162-7.924,5.319-15.645,9.774-23.276 c4.473-7.649,10.264-15.209,17.77-22.715l12.327-12.328L374.975,0l-12.336,12.319c-11.554,11.553-20.056,23.72-25.98,36.34 c-8.929,18.962-11.989,38.893-11.767,58.842c0.187,19.967,3.504,40.041,7.321,60.204c2.49,13.136,4.642,25.775,5.843,37.844 c1.805,18.117,1.441,34.874-2.864,50.696c-2.152,7.916-5.31,15.644-9.774,23.284c-4.474,7.64-10.264,15.191-17.77,22.698v0.009 h0.009l-5.417,5.416c-1.449,1.441-2.882,2.82-4.349,4.145l-88.637-88.637c-4.376,4.82-8.068,9.748-11.216,14.915l85.134,85.124 c-3.762,2.402-7.534,4.482-11.331,6.261c-4.207,1.984-8.449,3.611-12.772,4.91l-71.365-71.365 c-1.708,7.16-2.624,14.835-2.793,23.355l52.207,52.199c-4.535,0.4-9.16,0.542-13.883,0.516 c-10.575-0.106-21.603-1.228-33.112-2.979c0.48,2.632,0.943,5.158,1.414,7.622c1.147,5.967,2.285,12.238,3.335,18.722 c3.94,0.552,7.862,1.014,11.784,1.405c20.003,1.984,40.041,1.743,59.572-3.558c9.766-2.641,19.345-6.564,28.639-12.007 c9.285-5.434,18.303-12.398,26.949-21.043l5.407-5.425c11.554-11.536,20.047-23.711,25.997-36.34 c8.921-18.963,11.971-38.876,11.767-58.834c-0.205-19.967-3.514-40.05-7.32-60.204C355.159,153.288,353.015,140.659,351.815,128.59 z" /> <path className="st0" d="M492.42,117.463l-12.319,12.318c-1.458,1.45-2.89,2.829-4.349,4.145L387.106,45.28 c-4.367,4.83-8.058,9.766-11.207,14.915l85.124,85.134c-3.762,2.41-7.516,4.482-11.322,6.261c-4.207,1.983-8.458,3.61-12.771,4.909 l-71.365-71.356c-1.708,7.15-2.624,14.826-2.784,23.346l52.198,52.199c-4.536,0.409-9.16,0.543-13.884,0.516 c-10.574-0.098-21.603-1.227-33.112-2.988c0.48,2.633,0.933,5.177,1.422,7.64c1.13,5.967,2.269,12.228,3.327,18.713 c3.94,0.551,7.853,1.014,11.775,1.406c20.012,1.982,40.059,1.751,59.573-3.558c9.765-2.641,19.353-6.555,28.647-12.006 c9.276-5.425,18.295-12.399,26.957-21.053l12.31-12.327L492.42,117.463z" /> <path className="st0" d="M136.1,451.805L50.984,366.69c3.745-2.428,7.516-4.509,11.313-6.279c4.216-1.984,8.458-3.62,12.772-4.91 l71.365,71.366c1.708-7.151,2.624-14.836,2.784-23.356L97.038,351.33c4.536-0.4,9.152-0.569,13.874-0.534 c10.566,0.098,21.604,1.245,33.086,2.998c-0.462-2.641-0.924-5.194-1.396-7.649c-1.138-5.959-2.268-12.238-3.326-18.722 c-3.932-0.516-7.844-0.987-11.767-1.379c-20.021-2.01-40.076-1.779-59.599,3.531c-9.729,2.669-19.344,6.582-28.638,12.016 c-9.276,5.425-18.295,12.389-26.949,21.052L0.005,374.961l19.576,19.585l12.318-12.327c1.45-1.45,2.9-2.828,4.359-4.145 l88.646,88.638C129.261,461.892,132.969,456.956,136.1,451.805z" /> <path className="st0" d="M173.953,306.46c-1.788-18.116-1.441-34.873,2.855-50.686c2.161-7.942,5.31-15.653,9.783-23.293 c4.464-7.64,10.263-15.208,17.77-22.715l5.416-5.408l-0.009-0.017c1.441-1.45,2.891-2.82,4.358-4.128l88.629,88.629 c4.367-4.821,8.076-9.766,11.198-14.906l-85.116-85.115c3.753-2.428,7.533-4.51,11.322-6.28c4.216-1.974,8.458-3.62,12.772-4.91 l71.365,71.357c1.717-7.151,2.624-14.818,2.784-23.338l-52.181-52.19c4.527-0.4,9.152-0.569,13.874-0.524 c10.558,0.098,21.604,1.245,33.086,2.98c-0.462-2.633-0.924-5.177-1.396-7.64c-1.138-5.968-2.277-12.23-3.326-18.713 c-3.923-0.534-7.844-1.004-11.767-1.396c-20.029-1.993-40.076-1.761-59.599,3.548c-9.73,2.659-19.344,6.573-28.638,12.007 c-9.276,5.434-18.295,12.389-26.949,21.052l0.009,0.009l-5.416,5.416c-11.544,11.544-20.047,23.711-25.98,36.341 c-8.929,18.962-11.989,38.884-11.767,58.824c0.188,19.976,3.504,40.059,7.311,60.213c2.499,13.137,4.651,25.774,5.852,37.835 c1.796,18.135,1.441,34.873-2.864,50.704c-2.152,7.916-5.31,15.644-9.766,23.285c-4.474,7.64-10.282,15.182-17.779,22.698 l-12.318,12.327L137.034,512l12.336-12.318c11.544-11.544,20.047-23.72,25.988-36.341c8.921-18.962,11.98-38.875,11.767-58.833 c-0.205-19.976-3.522-40.041-7.329-60.195C177.297,331.167,175.153,318.538,173.953,306.46z" />
            </svg>

        ),
    },
    {
        label: 'أمراض الذكورة',
        description:
            'رعاية متخصصة وتشخيصات دقيقة مدعومة بعلاجات متقدمة لتحسين جودة الحيوانات المنوية وصحة الجهاز التناسلي لدى الرجال. ',
        icon: (
            <svg className="h-8" viewBox="0 0 64 64" xmlns="http://www.w3.org/2000/svg" strokeWidth={3} stroke="currentColor" fill="none"><g id="SVGRepo_bgCarrier" strokeWidth={0} />
                <circle cx={32} cy="13.48" r="5.91" strokeLinecap="round" />
                <path d="M25.48,56.43V43.83a2.18,2.18,0,0,0-.73-1.64l-2.19-2a1.11,1.11,0,0,1-.36-.82v-13a2.21,2.21,0,0,1,2.2-2.21H38.5a3.31,3.31,0,0,1,3.3,3.31v12a1.14,1.14,0,0,1-.3.76l-2.11,2.25a2.18,2.18,0,0,0-.6,1.51V56.43" strokeLinecap="round" />
            </svg>

        ),
    },
    {
        label: 'حفظ الخصوبة',
        description:
            'خيارات متقدمة لحفظ الخصوبة باستخدام أحدث التقنيات للمرأة والرجل – لمساعدتكم على حماية فرصتكم للإنجاب مستقبلاً بثقة وراحة. ',
        icon: (
            <svg fill="currentColor" className="h-6 w-6" version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink" viewBox="0 0 512 512" xmlSpace="preserve"><g id="SVGRepo_bgCarrier" strokeWidth={0} /><g id="SVGRepo_tracerCarrier" strokeLinecap="round" strokeLinejoin="round" /><g id="SVGRepo_iconCarrier"> <g> <g> <path d="M255.998,179.2c-28.228,0-51.2,22.972-51.2,51.2s22.972,51.2,51.2,51.2s51.2-22.972,51.2-51.2 S284.227,179.2,255.998,179.2z M255.998,256c-14.14,0-25.6-11.46-25.6-25.6c0-14.14,11.46-25.6,25.6-25.6 c14.14,0,25.6,11.46,25.6,25.6C281.598,244.54,270.138,256,255.998,256z" /> </g> </g> <g> <g> <path d="M510.164,400.094l-47.718-119.296l22.63-67.9c2.603-7.808,1.297-16.393-3.516-23.066 c-4.804-6.673-12.535-10.633-20.762-10.633h-102.4c-8.226,0-15.957,3.959-20.77,10.633c-4.813,6.673-6.118,15.258-3.516,23.066 l22.63,67.9l-34.065,85.171l-16.247-64.981c-2.85-11.392-13.082-19.388-24.832-19.388h-25.6h-25.6 c-11.75,0-21.982,7.996-24.832,19.388l-4.898,19.575l-21.879-120.346c-2.21-12.169-12.817-21.018-25.19-21.018h-102.4 c-12.373,0-22.972,8.849-25.182,21.026l-25.6,140.8c-1.357,7.467,0.666,15.155,5.53,20.983c4.864,5.828,12.066,9.19,19.652,9.19 h16.358l9.327,117.231C52.342,501.734,63.444,512,76.798,512h51.2c13.355,0,24.465-10.266,25.523-23.569L162.84,371.2h16.358 c3.26,0,6.366-0.896,9.327-2.074l-8.567,34.27c-1.911,7.646-0.196,15.753,4.659,21.965c4.855,6.204,12.297,9.839,20.181,9.839 v51.2c0,14.14,11.46,25.6,25.6,25.6h51.2c14.14,0,25.6-11.46,25.6-25.6v-51.2c4.54,0,8.747-1.485,12.587-3.695 c3.9,2.321,8.337,3.695,13.013,3.695h25.6v51.2c0,14.14,11.46,25.6,25.6,25.6h51.2c14.14,0,25.6-11.46,25.6-25.6v-51.2h25.6 c8.499,0,16.435-4.215,21.197-11.247C512.357,416.922,513.321,407.979,510.164,400.094z M139.194,345.6l-11.196,140.8h-51.2 L65.603,345.6H25.598l25.6-140.8h102.4l25.6,140.8H139.194z M281.598,409.6v76.8h-51.2v-76.8h-25.6l25.6-102.4h51.2l25.6,102.4 H281.598z M435.198,409.6v76.8h-51.2v-76.8h-51.2l51.2-128l-25.6-76.8h102.4l-25.6,76.8l51.2,128H435.198z" /> </g> </g> <g> <g> <path d="M409.598,0c-42.351,0-76.8,34.449-76.8,76.8c0,42.351,34.449,76.8,76.8,76.8s76.8-34.449,76.8-76.8 C486.398,34.449,451.949,0,409.598,0z M409.598,128c-28.279,0-51.2-22.921-51.2-51.2s22.921-51.2,51.2-51.2 c28.279,0,51.2,22.921,51.2,51.2S437.878,128,409.598,128z" /> </g> </g> <g> <g> <rect x="76.798" y="76.8" width="0.001" height="0.001" /> </g> </g> <g> <g> <path d="M102.398,0c-42.342,0-76.8,34.449-76.8,76.8c0,42.351,34.458,76.8,76.8,76.8c42.351,0,76.8-34.449,76.8-76.8 C179.198,34.449,144.749,0,102.398,0z M102.398,128c-28.279,0-51.2-22.921-51.2-51.2s22.921-51.2,51.2-51.2s51.2,22.921,51.2,51.2 S130.678,128,102.398,128z" /> </g> </g> </g></svg>

        ),
    },
    {
        label: 'التلقيح داخل الرحم',
        description: 'يُعد حقن الحيوانات المنوية داخل الرحم أحد خيارات علاج الخصوبة.',
        icon: (
            <svg className="h-8" version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 100 100" xmlSpace="preserve">
                <g>
                    <path fill="currentColor" d="M31.46,10.8c0,0,3.62,2.8,3.31,13.84c0,0-0.72,7.04,5.99,11.78c0,0,0.83,1.95,0.52,5.56
		c0,0-2.06,11.21,2.96,11.9c-0.1,4.63-0.2,9.26-0.3,13.88" />
                    <path fill="currentColor" d="M27.38,26.77c0,0,5.73-1.09,7.36-5.29" />
                    <path fill="currentColor" d="M68.64,10.8c0,0-3.62,2.8-3.31,13.84c0,0,0.72,7.04-5.99,11.78c0,0-0.83,1.95-0.52,5.56
		c0,0,2.06,11.21-2.96,11.9c-0.01,4.76-0.02,9.52-0.03,14.27" />
                    <path fill="currentColor" d="M72.71,26.77c0,0-5.73-1.09-7.36-5.29" />
                    <path fill="currentColor" d="M78.66,23.16c-3.03-0.3-5.52,1.45-5.95,3.62c-0.38,1.91,1.03,3.52,1.25,3.76
		c2.12,2.34,5.56,1.5,5.73,1.46c2.26-0.59,4.03-2.49,4.63-4.92c0.71-0.37,1.73-1.04,2.58-2.17c2.24-2.99,1.77-7.25,0.21-10.11
		c-2.14-3.93-6.21-4.98-6.97-5.16c-4.17-0.49-7.3-0.12-9.49,0.36c-1.96,0.43-5.35,1.43-9.02,0.04c-0.03-0.01-0.04-0.02-0.11-0.04
		c-0.44-0.17-0.91-0.44-1.41-0.78c-6.08-4.08-14.02-4.09-20.09,0c-0.5,0.34-0.97,0.61-1.41,0.78c-0.06,0.02-0.07,0.03-0.11,0.04
		c-3.67,1.39-7.07,0.39-9.02-0.04c-2.19-0.48-5.32-0.85-9.49-0.36c-0.75,0.18-4.82,1.23-6.97,5.16c-1.56,2.86-2.04,7.12,0.21,10.11
		c0.85,1.13,1.87,1.79,2.58,2.17c1.16,3.13,3.71,5.2,6.34,5.21c0.53,0,2.72-0.08,4.17-1.84c0.18-0.22,1.47-1.83,1.09-3.66
		c-0.43-2.09-2.89-3.79-5.95-3.62" />
                    <path fill="currentColor" d="M45.65,47V35.75l-1.59-1.64c-2.59-2.42-3.12-4.51-3.18-8.64c-0.03-2.29-0.85-3.74-1.74-4.63
		c-0.53-0.54-0.08-1.46,0.67-1.39c0.47,0.04,1.01,0.1,1.6,0.19c2.47,0.38,4.98,0.61,7.49,0.61h2.42c2.52,0,5.02-0.23,7.49-0.61
		c0.59-0.09,1.12-0.15,1.6-0.19c0.75-0.06,1.21,0.86,0.67,1.39c-0.89,0.89-1.7,2.34-1.74,4.63c-0.06,4.18-0.61,6.27-3.28,8.73
		l-1.28,1.13V47" />
                    <g>
                        <line fill="currentColor" x1="50.06" y1="63.74" x2="50.1" y2="28.25" />
                        <line fill="currentColor" x1="47.68" y1="93.84" x2="52.44" y2="93.85" />
                        <polygon fill="currentColor" points="51.84,86.67 48.26,86.67 48.26,66.21 50.07,63.76 51.84,66.28 		" />
                        <line fill="currentColor" x1="50.04" y1="93.84" x2="50.04" y2="86.67" />
                        <line fill="currentColor" x1="48.26" y1="86.67" x2="45.03" y2="86.67" />
                        <line fill="currentColor" x1="55.06" y1="86.67" x2="51.84" y2="86.67" />
                    </g>
                </g>
            </svg>
        ),
    },
];

export default function ServicesSection({ onBookNow }: ServicesSectionProps) {
    const [more, setMore] = React.useState(false);
    return (
        <section id="services" className="py-16 md:py-24 bg-[#f4f9fc]">
            <div className="px-3 md:px-6 mx-auto max-w-7xl">
                <div className="mx-auto mb-10 w-full max-w-4xl sm:px-12">
                    <h2 className="mb-1 text-2xl font-bold leading-snug text-center text-black lg:text-4xl sm:text-5xl">
                        الجيل القادم من رعاية الخصوبة
                    </h2>
                    <p className="mx-auto mb-2 mt-4 max-w-xl text-base text-center lg:text-base">
                        تقدم بنــــون رعاية شاملة لمشاكل الخصوبة وتأخر الإنجاب باستخدام أحدث التقنيات وبنسب نجاح
                        عالية تضاهي المعايير العالمية.
                    </p>
                </div>

                <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
                    {SERVICES.map((service, index) => (
                        <div
                            key={service.label}
                            className={" flex-col h-full p-4 md:p-10 bg-white shadow-sm rounded md:rounded-3xl text-right" + (index > 2 && !more ? " hidden md:flex" : " flex")}
                        >
                            <div className="flex flex-row-reverse items-center gap-3 w-full mb-6">
                                <div className="flex items-center">
                                    <div className="flex items-center justify-center w-14 h-14 bg-[#f0f7fb] rounded-2xl text-[#0ea5e9]">
                                        {service.icon}
                                    </div>
                                </div>
                                <h3 className="text-lg font-bold flex-1 text-[#0a192f]">{service.label}</h3>
                            </div>
                            <p className="flex-grow mb-8 text-sm leading-relaxed text-zinc-500">
                                {service.description}
                            </p>
                        </div>
                    ))}
                    {!more ? <button
                        onClick={(e) => {
                            e.preventDefault();
                            setMore((m) => !m);
                        }}
                        className="flex md:hidden col-span-full mx-auto px-4 py-1 bg-[#133e51] text-white rounded-md text-sm font-medium hover:bg-[#0b83a5] transition-colors"
                    >
                        عرض المزيد
                    </button> : null}
                </div>
            </div>
        </section>
    );
}
